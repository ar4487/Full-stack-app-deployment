...
authForm.addEventListener("submit", async (e) => {
  ...
  try {
    ...
    console.error(err);
    authMsg.textContent = err.detail
      ? err.detail
...
noteForm.addEventListener("submit", async (e) => {
  ...
  if (editingId) {
    ...
    r = await fetch(`${API}/notes/${editingId}`, {
      method: "PUT",
      headers: {
    ...
    delete noteForm.dataset.editingId; // clear edit state
  } else {
    // Create new note
  ...
  if (r.ok) {
    $("#note-title").value = "";
    $("#note-content").value = "";
    saveNoteBtn.textContent = "Add note"; // reset button text
...
async function loadNotes() {
  ...
  data.forEach((n) => {
    ...
    li.className = "note";
    li.innerHTML = `<div><h3>${escapeHtml(n.title)}</h3><p>${escapeHtml(
      n.content || ""
    )}</p></div>
      <div> <button data-id="${n.id}" class="edit">Edit</button>
      <button data-id="${n.id}" class="del">Delete</button></div>`;
    notesList.appendChild(li);
  });
  document.querySelectorAll(".del").forEach((btn) => {
    btn.addEventListener("click", async (e) => {
      const id = e.target.dataset.id;
      const rr = await fetch(`${API}/notes/${id}`, {
        method: "DELETE",
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  document.querySelectorAll(".edit").forEach((btn) => {
    btn.addEventListener("click", (e) => {
      ...
      noteForm.dataset.editingId = id; // mark form as editing
